...
export default function ShopTheLookCouples() {
  const { data: looks, isLoading } = useListLooks();
  const visibleLooks = (looks ?? []).filter((look: any) => (look.lookGroup ?? "main") === "couples");
    ...
    <div className="pb-24">
      ...
      <div className="container mx-auto px-4 sm:px-6 mt-8">
        <div className="no-scrollbar flex items-center gap-8 overflow-x-auto text-[10px] tracking-[0.35em] uppercase text-muted-foreground">
          <Link href="/shop-the-look" className="hover:text-foreground pb-2">Main</Link>
          <Link href="/shop-the-look-kids" className="hover:text-foreground pb-2">Kids</Link>
          <Link href="/shop-the-look-couples" className="text-foreground border-b border-foreground pb-2">Couples</Link>
        </div>
      </div>
